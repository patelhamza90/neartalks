...
function AppContent() {
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-gray-50">
      ...
      {/* Top App Bar */}
      <header className="bg-white border-b px-4 py-3 flex items-center gap-3 flex-shrink-0 shadow-sm w-full">
        {/* Logo */}
        ...
        <button
          ...
          className="flex-1 min-w-0 flex items-center gap-2 bg-gray-100 hover:bg-gray-200 border border-transparent hover:border-gray-300 rounded-xl px-3 py-2 text-sm text-gray-400 text-left transition max-w-md"
        >
          <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z" />
          </svg>
          <span className="flex-1 truncate">Search groups...</span>
          <kbd className="hidden sm:inline-flex items-center text-xs text-gray-400 bg-white border border-gray-200 rounded px-1.5 py-0.5 flex-shrink-0">⌘K</kbd>
        </button>
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-auto">
          <img
            ...
            alt="" className="w-8 h-8 rounded-full bg-gray-100"
          />
          <button
            onClick={logout}
            className="flex items-center gap-1.5 bg-red-50 hover:bg-red-100 text-red-500 font-semibold text-sm px-3 py-1.5 rounded-lg border border-red-200 transition"
          >
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden w-full">
        ...
        {/* Sidebar */}
        <div className={`flex flex-col bg-white border-r flex-shrink-0 overflow-hidden
        ...
        {/* Chat Panel */}
        <div className={`flex-1 min-w-0 flex flex-col overflow-hidden ${!showChat ? 'hidden md:flex' : 'flex'}`}>
          <ChatWindow
            group={activeGroup}
            onBack={handleBack}
            onLeaveGroup={() => { setActiveGroup(null); setShowChat(false); setTab('chats'); }}
          />
        </div>
      </div>
      ...
      {showGlobalSearch && (
        <GlobalSearch
          ...
          onSelectGroup={(group) => { handleSelectGroup(group); setTab('chats'); }}
          onGroupJoin={(group) => setJoinFromSearch(group)}
          onClose={() => setShowGlobalSearch(false)}
        />
      )}

      {/* Join modal from global search */}
      {joinFromSearch && (
        <JoinGroupModal
          group={joinFromSearch}
          onSuccess={(group, nickname) => { setJoinFromSearch(null); handleGroupJoined({ ...group, nickname }); }}
